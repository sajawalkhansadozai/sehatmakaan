# 💳 Complete Payment Flows Analysis - Sehat Makaan App

---

## **Overview**

**3 Main Payment Flows** in the app (all use **PayFast** gateway):

1. ✅ **Workshop Registration Payment** (Participant pays to join workshop)
2. ✅ **Workshop Creation Fee Payment** (Doctor pays PKR 10,000 to activate workshop)
3. ✅ **Booking Payment** (Patient pays for doctor appointment booking)

---

## **1️⃣ WORKSHOP REGISTRATION PAYMENT**

### **Where It Happens:**
- **File:** `lib/features/workshops/screens/user/workshop_registration_page.dart`
- **Checkout Page:** `lib/features/workshops/screens/user/workshop_checkout_page.dart` ✅ **JUST FIXED**
- **Webhook:** `functions/index.js` → `handlePayFastWebhook` (Line 3335)

### **Complete Flow:**

```
User clicks "Join Workshop"
    ↓
Registration Form Filled
├─ First name, Last name
├─ Email, CNIC, Phone
├─ Institution, Specialty
└─ Notes (optional)
    ↓
"Proceed to Payment" Button
    ↓
Workshop Checkout Page
├─ Order Summary (workshop details + price)
├─ Terms & Conditions checkbox
└─ "Proceed to Payment" button
    ↓
PayFast Payment Gateway Opens (External)
├─ Merchant ID: 10029646
├─ Merchant Key: qzffl86tqx6qk
├─ Amount: Workshop Price (PKR)
└─ Item: Workshop Title
    ↓
User Completes Payment on PayFast
    ↓
PayFast Sends Webhook Callback ✅ **REAL-TIME LISTENER NOW ADDED**
├─ Webhook URL: https://us-central1-sehatmakaan-833e2.cloudfunctions.net/handlePayFastWebhook
├─ Payment Status: COMPLETE
└─ Custom Fields: registrationId, paymentId, type=workshop
    ↓
Firestore Updates (Webhook Handler)
├─ workshop_registrations.status: 'pending' → 'confirmed' ✅
├─ workshop_payments.status: 'pending' → 'paid'
├─ Generates registration number: WS-YYYY-TIMESTAMP
└─ Increments workshop.currentParticipants (prevents overbooking)
    ↓
✅ Auto-Confirmation on App (Real-time Listener)
├─ Dialog closes automatically
├─ Success toast shown
└─ Navigation back to workshops page
    ↓
"My Joined Workshops" Section Shows
├─ Green "✅ Confirmed" badge appears
├─ Workshop title, price, description
└─ Countdown timer to workshop date
```

### **Key Data Saved:**

**collection: `workshop_registrations`**
```dart
{
  'workshopId': 'workshop_id',
  'userId': 'user_id',
  'firstName': 'John',
  'lastName': 'Doe',
  'email': 'john@example.com',
  'cnic': '12345-1234567-1',
  'phone': '0300-1234567',
  'institution': 'ABC Hospital',
  'specialty': 'Cardiology',
  'notes': 'Optional notes',
  'status': 'confirmed',                    // Updated by webhook
  'paymentStatus': 'paid',                  // Updated by webhook
  'registrationNumber': 'WS-2026-123456',   // Generated by webhook
  'paymentId': 'payfast_payment_id',
  'confirmedAt': Timestamp,                 // Set by webhook
  'registeredAt': Timestamp,
  'updatedAt': Timestamp
}
```

**collection: `workshop_payments`**
```dart
{
  'registrationId': 'registration_id',
  'workshopId': 'workshop_id',
  'userId': 'user_id',
  'amount': 5000,  // PKR
  'status': 'paid',                    // Updated by webhook
  'paymentMethod': 'payfast',
  'paymentId': 'payfast_payment_id',   // Updated by webhook
  'paidAt': Timestamp,                 // Set by webhook
  'createdAt': Timestamp
}
```

### **What's New (FIXED):**
✅ Real-time listener added to detect payment confirmation
✅ Auto-closes pending dialog when status changes
✅ Shows success toast automatically
✅ Auto-navigates back to workshops page

---

## **2️⃣ WORKSHOP CREATION FEE PAYMENT**

### **Where It Happens:**
- **File:** `lib/features/workshops/screens/user/workshop_creation_fee_checkout_page.dart`
- **Route:** `/workshop-creation-fee-checkout`
- **Webhook:** `functions/index.js` → `payfastWorkshopCreationWebhook` (Line 473)

### **Complete Flow:**

```
Doctor Creates Workshop (Form Filled)
    ↓
Submits Workshop for Admin Approval
├─ Workshop status: 'pending_admin'
└─ Waits for admin review
    ↓
Admin Approves Workshop ✅
├─ Workshop status: 'approved'
└─ Payment required before going live
    ↓
Doctor Sees "Pay Creation Fee" Button
├─ Amount: PKR 10,000 (Fixed)
└─ Must pay before workshop can receive participants
    ↓
"Pay Now" Button Clicked
    ↓
Workshop Creation Fee Checkout Page
├─ Workshop details summary (title, type, provider, date, time, price)
├─ Max participants
├─ "PKR 10,000 - One-time creation fee" highlighted
└─ "Proceed to Payment" button
    ↓
PayFast Payment Gateway Opens
├─ Amount: 10000 (Fixed)
├─ Item: Workshop Creation Fee - [Workshop Title]
└─ Custom field: type=workshop_creation
    ↓
Doctor Completes Payment
    ↓
PayFast Webhook Sent
├─ Status: COMPLETE
├─ Custom fields: workshopId, type=workshop_creation
└─ Webhook URL: https://us-central1-sehatmakaan-833e2.cloudfunctions.net/payfastWorkshopCreationWebhook
    ↓
Firestore Updates (Webhook)
├─ workshops.isActive: false → true ✅
├─ workshops.status: 'approved' → 'active'
├─ workshop_payments: payment marked as 'paid'
└─ Workshop now accepts registrations!
    ↓
Workshop Goes Live
├─ Appears in "Discover Workshops" for all users
├─ Users can now register and pay
└─ Doctor can manage registrations
```

### **Key Data Saved:**

**collection: `workshop_payments`** (Creation Fee)
```dart
{
  'workshopId': 'workshop_id',
  'creatorId': 'doctor_id',
  'amount': 10000,  // PKR (Fixed)
  'type': 'creation_fee',
  'status': 'paid',                    // Updated by webhook
  'paymentMethod': 'payfast',
  'paymentId': 'payfast_payment_id',   // Updated by webhook
  'paidAt': Timestamp,                 // Set by webhook
  'createdAt': Timestamp
}
```

**collection: `workshops`** (Updated)
```dart
{
  'isActive': true,          // Changed from false by webhook
  'status': 'active',        // Changed from 'approved' by webhook
  'creationFeeStatus': 'paid',
  'creationFeePaidAt': Timestamp,
  'updatedAt': Timestamp
}
```

---

## **3️⃣ BOOKING PAYMENT (Doctor Appointment)**

### **Where It Happens:**
- **File:** `lib/features/bookings/screens/user/booking_workflow_page.dart`
- **Payment Step:** `lib/features/payments/screens/payment_step.dart`
- **Webhook:** `functions/index.js` → `payfastWebhook` (Line 352)

### **Complete Flow:**

```
Patient Starts Booking (Suite → Type → Specialty)
    ↓
Date & Time Slot Selection
├─ Picks preferred date
├─ Selects time slot (hourly: 1-4 hours or specific time)
└─ Priority booking addon (for 6PM+ or weekend slots)
    ↓
Add-ons Selection (Optional)
├─ Priority Booking: PKR 5,000
├─ Extended Hours: PKR 2,500
└─ Additional services if any
    ↓
Booking Summary Review
├─ Base rate (suite specific)
├─ Priority multiplier (if applicable)
├─ Add-ons total
├─ Tax calculation
└─ Total amount (PKR)
    ↓
Payment Step
├─ Multiple payment methods available:
│  ├─ PayFast (Credit Card)
│  ├─ JazzCash (Mobile Wallet)
│  ├─ EasyPaisa (Mobile Wallet)
│  └─ Bank Transfer
├─ Patient selects PayFast
└─ Terms acceptance
    ↓
PayFast Payment Gateway Opens
├─ Amount: Total booking cost (PKR)
├─ Item: Doctor Booking - [Specialty]
└─ Custom fields: bookingId, registrationId
    ↓
Patient Completes Payment
    ↓
PayFast Webhook Sent
├─ Status: COMPLETE
└─ Webhook URL: https://us-central1-sehatmakaan-833e2.cloudfunctions.net/payfastWebhook
    ↓
Firestore Updates (Webhook)
├─ bookings.paymentStatus: 'pending' → 'paid' ✅
├─ bookings.status: 'confirmed'
├─ booking_payments.status: 'paid'
└─ Doctor sees booking in calendar
    ↓
Confirmation Email Sent to Patient
├─ Booking ID
├─ Doctor details
├─ Appointment time
├─ Cancellation policy
└─ Contact support link
    ↓
Patient Can View Booking
├─ In "My Bookings" section
├─ Can request cancellation (with refund rules)
└─ Get appointment reminders
```

### **Key Data Saved:**

**collection: `bookings`**
```dart
{
  'userId': 'patient_id',
  'doctorId': 'doctor_id',
  'suiteId': 'suite_id',
  'bookingType': 'hourly',          // or 'monthly_subscription'
  'specialty': 'Cardiology',
  'baseRate': 5000,
  'package': 'standard',             // for monthly
  'totalHours': 2,                   // for hourly
  'startTime': '14:00',
  'endTime': '16:00',
  'addonsTotal': 5000,               // Priority + Extended Hours
  'tax': 0,
  'totalAmount': 10000,
  'selectedAddons': [
    {'name': 'Priority Booking', 'code': 'priority_booking', 'price': 5000},
    {'name': 'Extended Hours', 'code': 'extended_hours', 'price': 2500}
  ],
  'bookingDate': Timestamp,
  'status': 'confirmed',             // Updated by webhook
  'paymentStatus': 'paid',           // Updated by webhook
  'paymentMethod': 'payfast',
  'paymentId': 'payfast_payment_id', // Added by webhook
  'createdAt': Timestamp,
  'confirmedAt': Timestamp
}
```

**collection: `booking_payments`**
```dart
{
  'bookingId': 'booking_id',
  'userId': 'patient_id',
  'amount': 10000,  // PKR
  'status': 'paid',                    // Updated by webhook
  'paymentMethod': 'payfast',
  'paymentId': 'payfast_payment_id',   // Updated by webhook
  'paidAt': Timestamp,                 // Set by webhook
  'payfastData': {...},                // Full PayFast response
  'createdAt': Timestamp
}
```

---

## **PAYMENT GATEWAY DETAILS**

### **PayFast Configuration:**

| Property | Value |
|----------|-------|
| **Merchant ID** | 10029646 |
| **Merchant Key** | qzffl86tqx6qk |
| **Payment URL** | https://www.payfast.co.za/eng/process |
| **Test Mode** | ❌ PRODUCTION (Not test mode) |
| **Currencies** | ZAR, PKR supported |

### **Webhook Endpoints:**

| Payment Type | Webhook Endpoint | Handler Function |
|--------------|-----------------|------------------|
| Workshop Registration | handlePayFastWebhook | Confirms registration, increments participants |
| Workshop Creation Fee | payfastWorkshopCreationWebhook | Activates workshop, makes it live |
| Booking Payment | payfastWebhook | Confirms booking, sends email |

### **Payment Flow Sequence:**

```
App → PayFast Gateway (External)
         ↓
User Pays on PayFast
         ↓
PayFast → Firebase Cloud Function (Webhook)
         ↓
Function Updates Firestore
         ↓
Real-time Listener in App Detects Change ✅ NOW ACTIVE
         ↓
App Shows Success & Auto-Navigates
```

---

## **⚠️ IMPORTANT NOTES**

### **1. Real-Time Confirmation (JUST FIXED):**
- **Before:** Users had to manually refresh to see payment confirmation
- **After:** Real-time listener auto-confirms when webhook updates Firestore
- **How:** Listener on registration document watches for `status === 'confirmed'`
- **Result:** Automatic dialog close + success toast + auto-navigation ✅

### **2. Payment Methods:**
- **Workshops:** Only **PayFast** (Credit card only)
- **Bookings:** Multiple options (PayFast, JazzCash, EasyPaisa, Bank Transfer)
- **Cart:** Not fully implemented yet (CheckoutPage expects CartItem[] but gets nothing)

### **3. Creation Fee:**
- **PKR 10,000** one-time fee per workshop
- Doctor must pay **before workshop goes live**
- Once paid: `workshops.isActive = true` and participants can register

### **4. Addon System:**
- Priority Booking: PKR 5,000 (access to 6PM+ and weekend slots)
- Extended Hours: PKR 2,500 (extend booking hours)
- Addons are optional but required for certain time slots

### **5. Security:**
- ⚠️ **Merchant credentials hardcoded** in source code (should use environment variables)
- ✅ Webhook verification implemented
- ✅ Transaction safety: Uses Firestore transactions to prevent overbooking

---

## **FLOW SUMMARY TABLE**

| Feature | Payment Type | Amount | Gateway | Status | Auto-Confirm |
|---------|-------------|--------|---------|--------|--------------|
| **Workshop Join** | Participant registration | Workshop price | PayFast | ✅ FIXED | ✅ YES |
| **Workshop Activate** | Creation fee | PKR 10,000 | PayFast | ✅ Working | ✅ YES |
| **Booking** | Appointment payment | Variable | PayFast+ | ✅ Working | ✅ YES |
| **Cart Checkout** | Product purchase | Variable | PayFast | ❌ Broken | ❌ NO |

---

## **Status Summary:**

✅ **Workshop Payment:** Working with real-time confirmation (just fixed)
✅ **Workshop Creation Fee:** Working 
✅ **Booking Payment:** Working
❌ **Cart Checkout:** Not wired properly (incomplete)

---

Generated: 27-Jan-2026
Last Updated: After payment confirmation fix
