...
class AuthService {
  ...
  Future<Map<String, dynamic>> registerDoctor({
    ...
    try {
      // PRE-VALIDATION: Check if PMDC or CNIC already exists
      final pmdcCheck = await _firestore
          .collection('users')
          .where('pmdcNumber', isEqualTo: pmdcNumber)
          .limit(1)
          .get();

      if (pmdcCheck.docs.isNotEmpty) {
        debugPrint('❌ Duplicate PMDC number detected: $pmdcNumber');
        return {
          'success': false,
          'error':
              'PMDC number already registered. Please use a unique PMDC number.',
        };
      }
      ...
      if (cnicCheck.docs.isNotEmpty) {
        debugPrint('❌ Duplicate CNIC number detected: $cnicNumber');
        return {
          'success': false,
          'error':
              'CNIC number already registered. Please use a unique CNIC number.',
        };
      }
      ...
      await _firestore.collection('users').doc(user.uid).set({
        ...
        'isActive': true, // ✅ DATA INTEGRITY: Set active status
        'isVerified': false,
        'isApproved': false,
