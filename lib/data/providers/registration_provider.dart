...
class RegistrationProvider with ChangeNotifier {
  ...
  void dispose() {
    ...
    _resendTimer?.cancel(); // ✅ Cancel timer
    super.dispose();
  }
  ...
  Future<String?> sendEmailOtp() async {
    ...
    try {
      // ✅ Clear old OTP from input field
  ...
  Future<String?> submitRegistration() async {
    ...
    try {
      ...
      final pmdc = pmdcController.text.trim();
      final cnic = cnicController.text.trim();

      // ✅ UNIQUE IDENTITY CHECK: Verify PMDC number
      final pmdcCheck = await _firestore
          .collection('users')
          .where('pmdcNumber', isEqualTo: pmdc)
          .limit(1)
          .get();

      if (pmdcCheck.docs.isNotEmpty) {
        isLoading = false;
        notifyListeners();
        return 'PMDC number already registered. Please use a unique PMDC number.';
      }

      // ✅ UNIQUE IDENTITY CHECK: Verify CNIC number
      final cnicCheck = await _firestore
          .collection('users')
          .where('cnicNumber', isEqualTo: cnic)
          .limit(1)
          .get();
      ...
      if (cnicCheck.docs.isNotEmpty) {
        isLoading = false;
        notifyListeners();
        return 'CNIC number already registered. Please use a unique CNIC number.';
      }
      ...
      // ✅ ATOMIC SIGNUP: Wrap Firestore write in try-catch
      try {
        ...
        await _firestore.collection('users').doc(userId).set({
          ...
          'userType': 'doctor', // ✅ DATA INTEGRITY: Set user type
          'status': 'pending',
          'createdAt': FieldValue.serverTimestamp(),
          'isActive':
              true, // ✅ DATA INTEGRITY: Set active (will be pending approval)
        ...
        debugPrint('❌ Firestore write failed, rolling back Auth user...');
        try {
        ...
        isLoading = false;
        notifyListeners();
        return 'Registration failed: Unable to create user profile. Please try again.';
      }
